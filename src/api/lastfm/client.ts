...
function md5(input: string): string {
  ...
  return binl2hex(computeMd5(str2binl(utf8), utf8.length * 8));
}
...
export function signParams(params: Record<string, string>, apiSecret: string): string {
  ...
  return md5(base + apiSecret);
}

export async function lastfmRequest<T = unknown>(
  params: Record<string, string>,
  config: { apiKey: string; apiSecret?: string; sessionKey?: string; signed?: boolean }
): Promise<T> {
  ...
  if (config.sessionKey) {
    fullParams.sk = config.sessionKey;
  }

  if (config.signed && config.apiSecret) {
    // api_sig is computed over all params except 'format'
    const { format: _f, ...signable } = fullParams;
    fullParams.api_sig = signParams(signable, config.apiSecret);
  }
  ...
  if (data.error) {
    throw new Error(data.message ?? `LastFM error ${data.error}`);
  }
