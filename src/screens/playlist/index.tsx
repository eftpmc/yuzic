...
const PlaylistView: React.FC = () => {
    ...
    useEffect(() => {
        const getPlaylist = async () => {
            try {
                setIsLoading(true);
                const collection = await api.playlists.get(id);
                if (!collection) {
    ...
    const isDownloaded = isPlaylistDownloaded(playlist);

    const isDownloading = isDownloadingPlaylist(playlist.id);
                ...
                renderItem={({ item }) => (
                    <View key={item.id} style={styles.songRow}>
                        <TouchableOpacity
                            ...
                            onPress={() => playSongInCollection(item, playlist)}
                        >
                    ...
                    <View style={styles.container}>
                        ...
                        <View style={styles.titleRow}>
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() => playSongInCollection(songs[0], playlist)}
                            >
